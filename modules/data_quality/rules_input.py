...
from config import BLOCK_ON_INVALID_LOCATION_STRUCTURE, EXPECTED_COLUMNS, DATE_SOURCE, DATE_SHIFT_DAYS
...
def validate_expected_columns(df: pd.DataFrame) -> tuple[bool, str]:
    ...
    expected_columns = [normalize_text(col) for col in EXPECTED_COLUMNS]
...
def extract_date_from_data(df: pd.DataFrame, filename: str = "") -> tuple[bool, str, str]:
    """
    Extrae la fecha de corte.
    Si config.DATE_SOURCE es 'filename' y se provee filename, intenta extraer la fecha del nombre (ej. YYYY-MM-DD).
    De lo contrario, extrae la fecha desde la columna 'Fecha Actualizacion' del DataFrame.
    Posteriormente aplica el desplazamiento config.DATE_SHIFT_DAYS (por defecto -1 día).
    ...
    if DATE_SOURCE == "filename" and filename:
        # Intentar YYYY-MM-DD
        match_yyyy = re.search(r"(\d{4}-\d{2}-\d{2})", filename)
        if match_yyyy:
            try:
                parsed_date = datetime.strptime(match_yyyy.group(1), "%Y-%m-%d")
            except ValueError:
                pass
        
        # Intentar DD-MM-YYYY
        if parsed_date is None:
            match_dd = re.search(r"(\d{2}-\d{2}-\d{4})", filename)
            if match_dd:
                try:
                    parsed_date = datetime.strptime(match_dd.group(1), "%d-%m-%Y")
                except ValueError:
                    pass

    if parsed_date is None:
        df_check = normalize_dataframe_columns(df)
        if "Fecha Actualización" not in df_check.columns:
            return False, "", "No se encontro la columna 'Fecha Actualizacion'"

        fechas = parse_datetime_series(df_check["Fecha Actualización"]).dropna()
        if fechas.empty:
            return False, "", "No se encontraron fechas validas en 'Fecha Actualizacion'"

        parsed_date = pd.to_datetime(fechas.iloc[0])

    if DATE_SHIFT_DAYS != 0:
        parsed_date = parsed_date + timedelta(days=DATE_SHIFT_DAYS)
...
def _prepare_location_structure_frame(df: pd.DataFrame) -> pd.DataFrame:
    df_check = normalize_dataframe_columns(df).copy()
    ...
    df_check["es_chavin"] = df_check["Almacén"].apply(is_controlled_internal_warehouse)
